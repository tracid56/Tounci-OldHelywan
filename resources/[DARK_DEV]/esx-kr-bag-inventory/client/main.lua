...
function Bag()
    ...
    table.insert(elements, {label = 'Mettre un object', value = 'put'})
    table.insert(elements, {label = 'Prendre objet', value = 'take'})
    table.insert(elements, {label = 'Jeter le sac', value = 'drop'})
    ...
    while true do
        ...
        for i=1, #Bags, 1 do
            ...
            if GetDistanceBetweenCoords(playercoords, Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z, true) <= 1.5 and not HasBag then
                ...
                Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.45, '~g~[E]~w~ ramasser le sac')
                Draw3DText(Bags[i].id.coords.x, Bags[i].id.coords.y, Bags[i].id.coords.z + 0.35, '~o~[N]~w~ Regarder dans le sac')
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustReleased(0, Keys['F2']) and HasBag and not IsPedInAnyVehicle(GetPlayerPed(-1), true) and not IsEntityInAir(PlayerPedId()) then -- Change F5 to the key you want to open the meny with
            Bag()
        end
    end
end)

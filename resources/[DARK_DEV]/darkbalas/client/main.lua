...
Citizen.CreateThread(function()
	...
	CacheApartments(function()
		for k,v in pairs(Apartments) do
			...
			local enterMessage = 'presse ~INPUT_CONTEXT~ Pour entrer dans la chambre du motel'

			if not owns then
				enterMessage = 'presse ~INPUT_CONTEXT~ Pour entrer dans la chambre du motel'
			end
			...
			if owned then
				...
				Markers.AddMarker('apartment_storage', v.storage, 'presse ~INPUT_CONTEXT~ Pour ouvrir le tiroir', function()
			...
			if owned or invited then
				...
				Markers.AddMarker('apartment_closet', v.closet, 'presse ~INPUT_CONTEXT~ Changer de vêtements ', function()
		    ...
		    Markers.AddMarker('apartment_exit', v.exit, 'presse ~INPUT_CONTEXT~ Sortir', function()
				local elements = {
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'apartment_storage_option',
		{
			...
			align = 'top-left',
			elements = {
				{label = 'Eşyaları Al', value = 'voir'},
				{label = 'Eşyaları Koy', value = 'mettre'}
			}
		},
		function(data, menu)
			menu.close()

			OpenStorageUnitContent(apartment, storage, data.current.value == 'Retirer')
		end,
		function(data, menu)
			menu.close()

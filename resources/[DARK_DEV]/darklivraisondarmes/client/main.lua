...
Citizen.CreateThread(function()
		...
		AddTextComponentString('Livraison d armes')
	...
	while true do
		...
		local v = Config.CargoProviderLocation 
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Press ~INPUT_CONTEXT~ interagir avec ~y~Dealer d armes")
		...
		Citizen.Wait(5000)
				if event_is_running then
						...
						if IsPedDeadOrDying(GetPlayerPed(-1)) then
							ResetCargo()
							DisplayMissionFailed('Tu es mort!')
						end

						if GetVehicleEngineHealth(event_vehicle) < 20 and event_vehicle ~= nil then
							ResetCargo()
							DisplayMissionFailed(' La cargaison a été sérieusement endommagée.')
						end

						if event_time_passed > 1800 then
							ResetCargo()
							DisplayMissionFailed('Livraison de la marchandise expirée.')
						end
...
function CargoMenu()
	ClearMenu()
    options.menu_title = "Dealer d armes"

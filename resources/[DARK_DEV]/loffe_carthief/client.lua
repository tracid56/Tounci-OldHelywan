...
local spawnPoint = Config.spawnPoint[math.random(#Config.spawnPoint)]
local dropPoint = Config.dropPoint[math.random(#Config.dropPoint)]
...
local cirklar = {
	[_U('start_hint')] = { ["x"] = 153.20, ["y"] = -3211.75, ["z"] = 5.00, ["mission"] = 0}
}
		...
		for k, v in pairs(cirklar) do
			...
			if GetDistanceBetweenCoords(coords, v["x"], v["y"], v["z"], true) < 100 then
				...
				if GetDistanceBetweenCoords(coords, v["x"], v["y"], v["z"], true) < 1.5 then
					...
					local thiefCoords = vector3(v["x"], v["y"], v["z"] + 1.2)
                    ESX.Game.Utils.DrawText3D(thiefCoords, k, 0.8)
...
AddEventHandler('loffe_carthief:mission0', function()
    ...
    dropPoint = Config.dropPoint[math.random(#Config.dropPoint)]
    spawnPoint = Config.spawnPoint[math.random(#Config.spawnPoint)]
    model = Config.Model[math.random(#Config.Model)]

	carThief = true
	--Wait(5000)
	Blip = AddBlipForCoord(spawnPoint.x, spawnPoint.y, spawnPoint.z)
	...
	repeat
		...
		DrawSubtitleTimed(500, 1)
			if GetDistanceBetweenCoords(coords, spawnPoint.x, spawnPoint.y, spawnPoint.z, true) < 50 then
				nearCar = 1
			end
		else
		ClearPrints()
...
Citizen.CreateThread(function()
	while true do
		...
		if carThief then
			...
			if GetDistanceBetweenCoords(coords, spawnPoint.x, spawnPoint.y, spawnPoint.z, true) < 50 then
				local spawned_cars = CreateVehicle(vehicleModel, spawnPoint.x, spawnPoint.y, spawnPoint.z, spawnPoint.h, true, false)
				SetVehicleOnGroundProperly(spawned_cars)
				local hasTjuvkopplat = 0
				repeat
					Citizen.Wait(500)
					if IsPedInVehicle(GetPlayerPed(-1), spawned_cars, false) then
						TriggerServerEvent('esx_phone:send', 'police', (_U('police_message')), true, {x = spawnPoint.x, y = spawnPoint.y, z = spawnPoint.z}, true)
						ClearPrints()
						SetTextEntry_2("STRING")
				...
				if IsPedInVehicle(GetPlayerPed(-1), spawned_cars, false) then
					ClearPrints()
					SetTextEntry_2("STRING")
					AddTextComponentString(_U('remove_transmitter', timesUntilSendareLoss))
				...
				Blip = AddBlipForCoord(dropPoint.x, dropPoint.y, dropPoint.z)
				SetBlipRoute(Blip, true)
				SetBlipColour(Blip, 11)
				SetBlipRouteColour(Blip, 11)
				...
				local coords = GetEntityCoords(GetPlayerPed(-1))
					if IsPedInVehicle(GetPlayerPed(-1), spawned_cars, false) then
							if GetDistanceBetweenCoords(coords, dropPoint.x, dropPoint.y, dropPoint.z, true) < 5 then

...
local zones = {
	{ ['x'] = 241.5701751709, ['y'] = -772.99774169922, ['z'] = 27.717334747314}
}
		...
		if dist <= 50.0 then  ------------------------------------------------------------------------------ Here you can change the RADIUS of the Safe Zone. Remember, whatever you put here will DOUBLE because 
			if not notifIn then																			  -- it is a sphere. So 50 will actually result in a diameter of 100. I assume it is meters. No clue to be honest.
				...
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#1E90FF'>Vous êtes dans une zone de sécurité</b>",
					type = "success",
					timeout = (3000),
					layout = "bottomcenter",
					queue = "global"
			...
			if not notifOut then
				...
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#1E90FF'>Vous n’êtes plus une zone de sécurité</b>",
					type = "error",
					timeout = (3000),
					layout = "bottomcenter",
					queue = "global"
		...
		DisablePlayerFiring(player,true) -- Disables firing all together if they somehow bypass inzone Mouse Disable
      	DisableControlAction(0, 106, true) -- Disable in-game mouse controls
			if IsDisabledControlJustPressed(2, 37) then --if Tab is pressed, send error message
				...
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#1E90FF'>Vous ne pouvez pas utiliser d'armes dans une zone de sécurité</b>",
					type = "error",
					timeout = (3000),
					layout = "bottomcenter",
					queue = "global"
			...
			if IsDisabledControlJustPressed(0, 106) then --if LeftClick is pressed, send error message
				...
				TriggerEvent("pNotify:SendNotification",{
					text = "<b style='color:#1E90FF'>Vous ne pouvez pas faire cela dans une zone de sécurité</b>",
					type = "error",
					timeout = (3000),
					layout = "bottomcenter",
					queue = "global"

    ...
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.DamageNeeded then
                    ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Appuyer sur [~g~LEFTSHIT et E~w~] pour pousser le vehicule', 0.8)
                end
                ...
                local currentVehicle = Vehicle.Vehicle
                 while true do
                    ...
                    if Vehicle.IsInFront then
                        SetVehicleForwardSpeed(currentVehicle, -1.1)
                    else
                        SetVehicleForwardSpeed(currentVehicle, 1.1)
                    end

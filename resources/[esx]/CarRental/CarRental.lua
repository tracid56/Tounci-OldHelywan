...
Citizen.CreateThread(function()
	...
	pickupStation = { --Set the car rental locaitons here
		{x = -1033.9533, y = -2659.2954, z = 12.8307},		--Airport car rental place
		{x = 254.14694213867, y = -773.46636962891, z = 29.606260299683},

	}
	
	dropoffStation = { --Set the car dropoff locations here
		{x = -1057.1934, y = -2644.9926, z = 12.8307}, --Airport car rental place
	...
	while true do
		...
		elseif WarMenu.IsMenuOpened('carPicker') then
			if WarMenu.Button('Glendale | Louer: $100 | Jours/1h: $100') then
				...
				isBeingCharged = true
				WarMenu.CloseMenu()
			elseif WarMenu.Button('Blista | Louer: $200 | Jours/1h: $100') then
				...
				isBeingCharged = true
				WarMenu.CloseMenu()
			elseif WarMenu.Button('Primo | Louer: $100 | Jours/1h: $100') then
				SpawnVehicle("primo")
				TriggerServerEvent("chargePlayer", 100)
				ESX.ShowNotification("Vous avez été facturé 100 $ pour votre location.")
				autoChargeAmount = 100
				isBeingCharged = true
				WarMenu.CloseMenu()
			elseif WarMenu.Button('Intruder | Louer: $250 | Jours/1h: $100') then
				SpawnVehicle("intruder")
				TriggerServerEvent("chargePlayer", 250)
				ESX.ShowNotification("On vous a facturé 250 $ pour votre location.")
				autoChargeAmount = 100
				isBeingCharged = true
				WarMenu.CloseMenu()
			elseif WarMenu.Button('Sultan | Louer: $1000 | Jours/1h: $300') then
				SpawnVehicle("sultan")

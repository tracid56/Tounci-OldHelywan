...
function Livraison()
	...
	if CurrentDelivery then
		...
		ESX.ShowAdvancedNotification('~r~GoPostal', '', '[Secteur De Ronde]\n----------------------\n'.. district.label, 'CHAR_BRYONY', 1 )
		ESX.ShowAdvancedNotification('~r~GoPostal', '', _U('create_itinary'), 'CHAR_ANDREAS', 1 )
...
function LivraisonStop(district, cancel)
	...
	if cancel then
		...
		if progress < district.long then
			...
			CurrentDelivery = false
			progress = 1
			ESX.ShowAdvancedNotification('~r~GoPostal', '', '[Information Ronde]\n----------------------\n'.. _U('finish_delivery'), 'CHAR_BRYONY', 1 )
		end
	end
end

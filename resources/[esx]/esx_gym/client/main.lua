...
local blips = {
	{title="Le Cercle", colour=7, id=311, x = -1201.2257, y = -1568.8670, z = 4.6101}
}
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(rentbike) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyer sur ~INPUT_CONTEXT~ pour louer un ~b~velo')
				
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Vous avez déjà un ~r~vehicule")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyer ~INPUT_CONTEXT~ pour ouvrir le ~b~gym~w~ menu')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyer ~INPUT_CONTEXT~ pour exercer vos ~g~bras')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("préparation de l' ~g~exersice~w~...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Tu as besoin de te reposer ~r~60 secondes ~w~avant de faire un autre exercice.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Vous avez besoin d'un abonnement pour faire une ~r~exercise")
						end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyer ~INPUT_CONTEXT~ pour faire des ~g~tractions')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyer ~INPUT_CONTEXT~ pour faire des ~g~pompes')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(situps) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Appuyer sur ~INPUT_CONTEXT~ pour faire des ~g~pompes')
    ...
    ESX.UI.Menu.Open(
        ...
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Nous sommes ouverts ~g~24~w~ heurs/ jours. Bienvenue!")
...
function OpenGymShopMenu()
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_shop_menu',
        {
            title    = 'Le Cercle',
            elements = {
				{label = 'Shake protéiné ($6)', value = 'protein_shake'},
...
function OpenGymShipMenu()
    ...
    ESX.UI.Menu.Open(
        ...
        {
            title    = 'Le Cercle - Membre',
            elements = {
				{label = 'Carte de Membre ($800)', value = 'membership'},
            }
        },
        function(data, menu)
